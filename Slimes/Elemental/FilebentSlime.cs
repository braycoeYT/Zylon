...
namespace Zylon.NPCs.Slimes.Elemental
{
	public class FilebentSlime : ModNPC
	{
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			...
			NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, NPCType<NPCs.Bosses.Minions.Spammer>(), 0, npc.whoAmI);
			if (Main.rand.Next(110) == 0)
			NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, NPCType<NPCs.Bosses.Minions.Cookie>(), 0, npc.whoAmI);
			if (Main.rand.Next(100) == 0)
			NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, NPCType<NPCs.Bosses.Minions.TheEmbeded>(), 0, npc.whoAmI);
		}
	}
}