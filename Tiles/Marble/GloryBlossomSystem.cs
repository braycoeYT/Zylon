...
{
	public class GloryBlossomSystem : ModSystem
	{
		...
		public static void AttemptBlossom()
		{
			...
			if (Main.tile[x, y].TileType != TileID.Marble && (int)(Main.tile[x, y].TileType) != ModContent.TileType<BloodLeaves>() && Main.tile[x, y].TileType != TileID.MarbleBlock)
			{
				return;
			}
			...
			WorldGen.PlaceTile(x, y - 1, ModContent.TileType<GloryBlossom>(), true, false, -1, 0);
