    ...
    public class Adenite_Center : ModNPC
	{
		...
		public override void HitEffect(NPC.HitInfo hit) {
			if (NPC.life > 0) {
				...
				for (int j = 0; j < 6; j++) Gore.NewGore(NPC.GetSource_FromAI(), NPC.Center, new Vector2(Main.rand.NextFloat(-5, 5), Main.rand.NextFloat(-6, 6)), ModContent.GoreType<Gores.Enemies.SpikeRingDeath>());
				Gore.NewGore(NPC.GetSource_FromAI(), NPC.Center, new Vector2(Main.rand.NextFloat(-2, 2), 0), ModContent.GoreType<Gores.Enemies.AdeniteGore>());
			}
		}
		int Timer;
		float yeah;
        ...
        public override void AI() {
			...
			if (!init) {
				//Projectile.NewProjectile(NPC.GetSource_FromNPC(), NPC.Center, new Vector2(), ModContent.ProjectileType<Projectiles.Enemies.Adenite_LaserEye>(), 0, 0, 255, NPC.whoAmI);
				NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.Center.X, (int)NPC.Center.Y, ModContent.NPCType<Adenite_LaserEye>(), 0, NPC.whoAmI, NPC.ai[0]);
				NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.Center.X, (int)NPC.Center.Y, ModContent.NPCType<Adenite_CenterDeco>(), 0, NPC.whoAmI);
				NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.Center.X, (int)NPC.Center.Y, ModContent.NPCType<Adenite_SpikeRing>(), 0, NPC.whoAmI);
				init = true;
			}
			/*if (help == 1f) {
				Timer++;
				if (Timer % 240 == 0)
					Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, Vector2.Normalize(NPC.Center - Main.player[NPC.target].Center) * -12f, ModContent.ProjectileType<Projectiles.Bosses.Adeneb.AdenebLaser>(), (int)(NPC.damage * 0.3f), 0f);
            }*/
