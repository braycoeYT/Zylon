...
namespace Zylon.NPCs.Desert
{
	public class DustbowlGigaslime : ModNPC
	{
        ...
        public override void AI() {
			...
			else if (Main.player[NPC.target].ZoneCorrupt)
				decide = ModContent.ProjectileType<Projectiles.Gigaslime.EbonsandballHostile>();
			else if (Main.player[NPC.target].ZoneCrimson)
				decide = ModContent.ProjectileType<Projectiles.Gigaslime.CrimsandballHostile>();
			else decide = ModContent.ProjectileType<Projectiles.Gigaslime.SandballHostile>();
			...
			if (NPC.velocity.Y == 0f || (NPC.Center.Y - Main.player[NPC.target].Center.Y > 300))
			{
				...
				if (num19 > 0)
				{
					...
					if (num19 == 3)
					{
						NPC.velocity.Y = -11f - yBoost; //-8
						NPC.velocity.X = NPC.velocity.X + (float)((8+xBoost) * NPC.direction); //3
						NPC.ai[0] = -200f;
						NPC.ai[3] = NPC.position.X;
					}
					else
					{
						NPC.velocity.Y = -8f - yBoost; //-6
						NPC.velocity.X = NPC.velocity.X + (float)((6+xBoost) * NPC.direction); //2
						NPC.ai[0] = -120f;
						if (num19 == 1)
						{
							NPC.ai[0] -= 1000f;
        ...
        public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry) {
			bestiaryEntry.Info.AddRange(new IBestiaryInfoElement[] {
				...
				new FlavorTextBestiaryInfoElement("This slime contains a mini sandstorm inside of itself, absorbing new sand to throw with each jump.")
			});
        ...
        public override float SpawnChance(NPCSpawnInfo spawnInfo) {
            ...
            return (SpawnCondition.OverworldDayDesert.Chance + SpawnCondition.DesertCave.Chance) * 0.0015f;
        }
		public override void ModifyNPCLoot(NPCLoot npcLoot) {
			...
			npcLoot.Add(new DropBasedOnExpertMode(new CommonDrop(ModContent.ItemType<Items.Spears.EyeoftheSandstorm>(), 2), new CommonDrop(ModContent.ItemType<Items.Spears.EyeoftheSandstorm>(), 1)));
			npcLoot.Add(new CommonDrop(ItemID.SandBlock, 1, 20, 30));
			npcLoot.Add(new CommonDrop(ItemID.Cactus, 1, 20, 30));
			npcLoot.Add(new CommonDrop(ItemID.AntlionMandible, 1, 3, 5));
			npcLoot.Add(new CommonDrop(ItemID.PinkPricklyPear, 3));
			npcLoot.Add(new CommonDrop(ItemID.DungeonDesertKey, 1000));
		}
    }
}