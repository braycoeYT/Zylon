...
{
	public class LittoralGigaslime2 : ModNPC
	{
        ...
        public override void HitEffect(NPC.HitInfo hit) {
            if (NPC.life < NPC.lifeMax / 2) {
				...
				NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.Center.X + 50, (int)NPC.Center.Y, ModContent.NPCType<LittoralGigaslime3>());
				if (Main.expertMode) NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.Center.X, (int)NPC.Center.Y, ModContent.NPCType<LittoralGigaslime3>());
				NPC.active = false;
            }
        }
        int xBoost;
        ...
        public override void AI() {
			...
			if (Timer % 20 == 0 && ((NPC.life < NPC.lifeMax * 0.875f) || Main.expertMode)) { // + (90*(NPC.life/NPC.lifeMax))) == 0 && (Main.expertMode || NPC.life > NPC.lifeMax / 2))
				ProjectileHelpers.NewNetProjectile(NPC.GetSource_FromThis(), NPC.Center, new Vector2(Main.rand.Next(-3, 4) + (Main.rand.Next(-3, 4) * NPC.life/NPC.lifeMax), -6 - Main.rand.Next(0, 5)), ModContent.ProjectileType<Projectiles.Gigaslime.WaterStreamHostile>(), (int)(NPC.damage * 0.2f), 0f, Main.myPlayer, BasicNetType: 2);
			...
			if ((NPC.velocity.Y == 0f || (NPC.Center.Y - Main.player[NPC.target].Center.Y > 300)) && Timer > NPC.ai[0] * 90)
			{
				...
				if (num19 > 0)
				{
					...
					if (num19 == 3)
					{
						NPC.velocity.Y = -13f - yBoost + (rand1*NPC.direction); //-11 og giga
					...
					{
						NPC.velocity.Y = -10f - yBoost + (rand1*NPC.direction); //-8 og giga
