...
{
	public class LittoralGigaslime3 : ModNPC
	{
		...
		/*public override void HitEffect(int hitDirection, double damage) {
            if (NPC.life < 1) {
				for (int i = 0; i < 3; i++)
				NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.Center.X + Main.rand.Next(-20, 21), (int)NPC.Center.Y - Main.rand.Next(0, 21), ModContent.NPCType<CyanSlime>());
            }
        }*/
        int xBoost;
		int yBoost;
		int Timer;
		int rand1 = Main.rand.Next(-2, 3);
		int rand2 = Main.rand.Next(-2, 3);
		float slimeAI0 = Main.rand.Next(-500, 501);
        public override void AI() {
			if (NPC.velocity.Y == 0 || (NPC.Center.Y - Main.player[NPC.target].Center.Y > 300) || Timer > NPC.ai[0] * 90) Timer++;
			...
			if (Timer % 20 == 0 && ((NPC.life < NPC.lifeMax * 0.875f) || Main.expertMode)) { // + (90*(NPC.life/NPC.lifeMax))) == 0 && (Main.expertMode || NPC.life > NPC.lifeMax / 2))
				ProjectileHelpers.NewNetProjectile(NPC.GetSource_FromThis(), NPC.Center, new Vector2(Main.rand.Next(-2, 3) + (Main.rand.Next(-1, 2) * NPC.life/NPC.lifeMax), -4 - Main.rand.Next(0, 3)), ModContent.ProjectileType<Projectiles.Gigaslime.WaterStreamHostile>(), (int)(NPC.damage * 0.2f), 0f, Main.myPlayer, BasicNetType: 2);
			...
			if ((NPC.velocity.Y == 0f || (NPC.Center.Y - Main.player[NPC.target].Center.Y > 300)) && Timer > NPC.ai[0] * 90)
			{
				...
				if (num19 > 0)
				{
					...
					if (num19 == 3)
					{
						NPC.velocity.Y = -11f - yBoost + (rand1*NPC.direction); //-11 og giga
						NPC.velocity.X = NPC.velocity.X + (float)((8+xBoost) * NPC.direction) + (rand2*NPC.direction); //3
						slimeAI0 = -200f;
						NPC.ai[3] = NPC.position.X;
					}
					else
					{
						NPC.velocity.Y = -8f - yBoost + (rand1*NPC.direction); //-8 og giga
						NPC.velocity.X = NPC.velocity.X + (float)((6+xBoost) * NPC.direction) + (rand2*NPC.direction); //2
						slimeAI0 = -120f;
						if (num19 == 1)
						{
							slimeAI0 -= 1000f;
