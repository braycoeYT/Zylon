        ...
        public override bool CanUseItem(Player player) {
			...
			if (player.altFunctionUse == 2) {
				...
				if (modeCharge) {
					Item.useTime = 2;
					Item.useAnimation = 2;
					CombatText.NewText(player.getRect(), Color.PaleGreen, "CHARGE");
                }
				else {
					...
					CombatText.NewText(player.getRect(), Color.PaleGreen, "SHOOT");
                }
				return false;
            }
            return true;
        ...
        public override bool? UseItem(Player player) {
			...
			if (modeCharge) {
				...
				if (chargeCount % 10 == 0 && charge != maxCharge + p.blowpipeMaxInc)
					CombatText.NewText(player.getRect(), Color.PaleGreen, (int)charge);
				else if (chargeCount % 10 == 0 && charge == maxCharge + p.blowpipeMaxInc)
					CombatText.NewText(player.getRect(), Color.PaleGreen, "MAX!");
            }
			else {
