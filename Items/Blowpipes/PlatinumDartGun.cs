        ...
        public override bool CanUseItem(Player player) {
			...
			if (player.altFunctionUse == 2) {
				...
				if (modeCharge) {
					Item.useTime = 2;
					Item.useAnimation = 2;
					CombatText.NewText(player.getRect(), Color.DimGray, "CHARGE");
                }
				else {
					...
					CombatText.NewText(player.getRect(), Color.DimGray, "SHOOT");
                }
				return false;
            }
            return true;
        ...
        public override bool? UseItem(Player player) {
			...
			if (modeCharge) {
				...
				if (chargeCount % 10 == 0 && charge != maxCharge + p.blowpipeMaxInc)
					CombatText.NewText(player.getRect(), Color.DimGray, (int)charge);
				else if (chargeCount % 10 == 0 && charge == maxCharge + p.blowpipeMaxInc)
					CombatText.NewText(player.getRect(), Color.DimGray, "MAX!");
            }
			else {
				charge = 0;
				chargeCount = 0;
