...
namespace Zylon.Items.Tools
{
	public class Matilda : ModItem
	{
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback) {
			...
			Projectile.NewProjectile(source, position, velocity.RotatedBy(MathHelper.ToRadians(-15*player.direction)), type, damage/2, knockback/2, player.whoAmI);
			Projectile.NewProjectile(source, position-(velocity*8), velocity, type, damage/2, knockback/2, player.whoAmI, 8);
			Projectile.NewProjectile(source, position-(velocity.RotatedBy(MathHelper.ToRadians(15*player.direction))*16), velocity.RotatedBy(MathHelper.ToRadians(15*player.direction)), type, damage/2, knockback/2, player.whoAmI, 16);
			return false;
        }
        public override void AddRecipes() {
			...
			recipe.AddRecipeGroup("Zylon:AnyShadowScale", 4);
