	...
	public class DiskitePremonition : ModProjectile
	{
        ...
        public override void AI() {
			...
			if (Timer == 1) {
				...
				if ((int)Projectile.ai[0] % 2 == 0) Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<DiskitePremonition2>(), 0, 0, Main.myPlayer);
            }
			if (Timer == 3) {
				if ((int)Projectile.ai[0] > 0) Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center + temp, temp, ModContent.ProjectileType<DiskitePremonition>(), 0, 0, Main.myPlayer, ((int)Projectile.ai[0] - 1));
				Projectile.velocity = Vector2.Zero;
			}
        }
