...
{
	public class AdenebPhaseSunBig : ModProjectile
	{
		...
		public override void OnHitPlayer(Player target, Player.HurtInfo info) {
            target.AddBuff(BuffID.OnFire, Main.rand.Next(7, 11) * 60);
        }
		int Timer;
		...
		public override bool PreDraw(ref Color lightColor) {
			...
			for (int i = 0; i < 8; i++) {
				int rot = Timer+i*45;
				Vector2 yellowPos = (Projectile.Center + new Vector2(0, 40).RotatedBy(MathHelper.ToRadians(rot)) + Vector2.UnitY * Projectile.gfxOffY - Main.screenPosition).Floor();
