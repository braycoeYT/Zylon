...
{
	public class AdenebMiniSunChaseFinale : ModProjectile
	{
        ...
        public override void AI() {
			...
			if (Timer < 60) {
				...
				rand = Main.player[owner.target].Center + new Vector2(Main.rand.Next(-200, 201), 0);
            }
			else if (Timer < 75) {
				Projectile.velocity += Vector2.Normalize(Projectile.Center - rand) * -0.18f; //-0.2 og
            }
			else if (Timer < 140) Projectile.velocity *= 1.02f; //1.04 og
        }
