...
{
	public class AlbinoMandibladeProj2 : ModProjectile
	{
        ...
        public override void AI() {
			Timer++;
			if (Timer == 1) {
				//Vector2 temp = Main.MouseWorld; //new Vector2(Projectile.ai[0], Projectile.ai[1]);
				/*int rot = 1;
				...
				Projectile.velocity = new Vector2(0, -12f).RotatedByRandom(MathHelper.TwoPi);
        ...
        public override bool PreDraw(ref Color lightColor) {
		    ...
		    Main.spriteBatch.Draw(projectileTexture, drawPos, new Rectangle?(new Rectangle(0, spriteSheetOffset, projectileTexture.Width, frameHeight)), Color.White, Projectile.rotation, drawOrigin, Projectile.scale, SpriteEffects.None, 0f);
		...
		public override void OnKill(int timeLeft) {
			...
			for (int i = 0; i < 3; i++) {
				Dust dust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.Firework_Pink);
				dust.noGravity = true;
				dust.scale = 1f;
			}
		}
