...
namespace Zylon.Projectiles.Spears
{
	public class DesertSpiritFlameFriendly : ModProjectile
	{
        ...
        public override void AI() {
			Lighting.AddLight(Projectile.Center, 0.2f*((255-Projectile.alpha)/255), 0.05f*((255-Projectile.alpha)/255), 0.33f*((255-Projectile.alpha)/255));
			Projectile.alpha -= 5;
			if (Projectile.alpha >= 100) return;
				float num165 = 16f; //(float)Math.Sqrt((double)(Projectile.velocity.X * Projectile.velocity.X + Projectile.velocity.Y * Projectile.velocity.Y));
				...
				if (Projectile.ai[1] > 0f)
				{
					...
					{
						float num176 = Main.npc[num175].Center.X;
						float num177 = Main.npc[num175].Center.Y;
						...
						{
							flag4 = true;
							num167 = Main.npc[num175].Center.X;
							num168 = Main.npc[num175].Center.Y;
						}
					}
					else
					{
