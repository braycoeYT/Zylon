...
namespace Zylon.Projectiles.Tomes
{
	public class MonsoonProj : ModProjectile
	{
		...
		public override void AI() {
			...
			if (Projectile.timeLeft == 240 && !bad) {
			...
			if (Projectile.timeLeft > 240) {
				if (Main.maxNPCs > 0 && Main.maxNPCUpdates > 0) {
					...
					for (npcCount = 0; npcCount < Main.maxNPCs; npcCount++) {
						if (Main.npc[npcCount].active && !Main.npc[npcCount].townNPC && !Main.npc[npcCount].friendly && Main.npc[npcCount].type != NPCID.TargetDummy && Main.npc[npcCount].life > 0) {
				...
				if (target.active == false || target.life < 0 || target.type == NPCID.TargetDummy || lowestDistance > 2000) {
