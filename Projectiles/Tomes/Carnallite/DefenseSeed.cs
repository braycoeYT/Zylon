	...
	public class DefenseSeed : ModProjectile
	{
        ...
        public override bool OnTileCollide(Vector2 oldVelocity)
        {
			...
			for (float d = 0; d < 6.28f; d += RotationPerLoop)
            {
				...
				Dust.NewDustPerfect(Projectile.Center, ModContent.DustType<Dusts.CarnalliteTome.DefenseDust>(), new Vector2(x, y).RotatedBy(RandomEndRotation) * Multiplier, 0, default, 1.3f);
			}
            ...
            {
				Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<DefenseFlower>(), 0, 0f, Main.myPlayer, 0f, 0f);
            }
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			Texture2D Glow = (Texture2D)ModContent.Request<Texture2D>("Zylon/Projectiles/Tomes/Carnallite/DefenseSeedGlow");
