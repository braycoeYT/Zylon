...
namespace Zylon.Projectiles.Tomes.Carnallite
{
	public class HealingFlower : ModProjectile
	{
		...
		readonly Color AOEColor1 = new Color(174, 255, 158);
		readonly Color AOEColor2 = new Color(174, 255, 158, 155);

		public override bool PreDraw(ref Color lightColor)
		{
			Texture2D projectileTexture = TextureAssets.Projectile[Projectile.type].Value;
			Texture2D leaves = (Texture2D)ModContent.Request<Texture2D>("Zylon/Projectiles/Tomes/Carnallite/Leaves");
			Texture2D floralGlow = (Texture2D)ModContent.Request<Texture2D>("Zylon/Projectiles/Tomes/Carnallite/HealingFlowerGlow");
		...
		public void Burst(Player player)
        {
			...
			if (Vector2.Distance(Projectile.Center, player.Center) <= AOERange)
			{
				...
				for (int d = 0; d < 3; d++)
                {
					Dust.NewDustDirect(player.Center, 0, player.height, ModContent.DustType<Dusts.CarnalliteTome.HealingFlowerBuffDust>(), Main.rand.NextFloat(-0.57f, 0.57f), 0f, 0, default, 1f);
