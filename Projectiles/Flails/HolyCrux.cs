...
namespace Zylon.Projectiles.Flails
{
	public class HolyCrux : ModProjectile
	{
		...
		public override void AI() {
			...
			float maxLaunchLength = 1250f; // How far the projectile's chain can stretch before being forced to retract when in launched state
			float retractAcceleration = 4f; // How quickly the projectile will accelerate back towards the player while retracting
			float maxRetractSpeed = 12.5f; // The max speed the projectile will have while retracting
			float forcedRetractAcceleration = 11.5f; // How quickly the projectile will accelerate back towards the player while being forced to retract
			...
			switch (CurrentAIState) {
				case AIState.Spinning: {
						...
						if (Timer % 9 == 0) {
							Vector2 speed = Main.player[Projectile.owner].Center.DirectionTo(Projectile.Center);
							if (Main.myPlayer == Projectile.owner) Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, speed*30f, ModContent.ProjectileType<HolyCruxClone>(), (int)(Projectile.damage*0.66f), Projectile.knockBack/2f, Projectile.owner);
						}
						break;
					}
			...
			if (CurrentAIState != AIState.Spinning) {
				...
				Vector2 projDir = Main.player[Projectile.owner].Center.DirectionTo(Projectile.Center)*30f;
				if (!foundTarget) {
					for (int i = 0; i < Main.maxNPCs; i++) {
						...
						if (npc.CanBeChasedBy()) {
							...
							if (((closest && inRange) || !foundTarget) && (lineOfSight || closeThroughWall)) {
								...
								projDir = Vector2.Normalize(targetCenter - Projectile.Center) * 30f;
							}
						}
					}
				}
				if (Timer % 10 == 0 && Main.myPlayer == Projectile.owner)
					Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, projDir, ModContent.ProjectileType<HolyCruxClone>(), (int)(Projectile.damage*0.66f), Projectile.knockBack/2f, Projectile.owner);
			}
		}
