...
{
	public class UniPet : ModProjectile
	{
        ...
        public override bool PreDraw(ref Color lightColor) {
            ...
            if (glitchTimer > 0 || blinkTimer > 0) projectileTexture = (Texture2D)ModContent.Request<Texture2D>("Zylon/Projectiles/Pets/UniPet_Red");
			int frameHeight = projectileTexture.Height / Main.projFrames[Projectile.type];
			int spriteSheetOffset = frameHeight * Projectile.frame;

		    Vector2 drawOrigin = new Vector2(Projectile.width * 0.5f, Projectile.height * 0.5f);
		    Vector2 drawPos = Projectile.Center - Main.screenPosition + new Vector2(0f, Projectile.gfxOffY);
			Color color = Color.White; //lightColor;

            if (Math.Abs(Projectile.rotation) > MathHelper.PiOver2) { //This took a stupid amount of time for me to figure out. Also, why doesn't sprite effects let you flip both?!
                projectileTexture = (Texture2D)ModContent.Request<Texture2D>("Zylon/Projectiles/Pets/UniPet_2");
                if (glitchTimer > 0 || blinkTimer > 0) projectileTexture = (Texture2D)ModContent.Request<Texture2D>("Zylon/Projectiles/Pets/UniPet_Red2");
            }
